...
/// `a + (-a) = 0 mod p`
///
pub fn inverse_addition(a: &BigUint, p: &BigUint) -> Result<BigUint, FiniteFieldError> {
...
pub fn subtract(a: &BigUint, b: &BigUint, p: &BigUint) -> Result<BigUint, FiniteFieldError> {
    ...
    let b_inv = inverse_addition(b, p)?;
...
pub fn divide(a: &BigUint, b: &BigUint, p: &BigUint) -> Result<BigUint, FiniteFieldError> {
    ...
    let d_inv = inverse_multiplicate_prime(b, p)?;

    multiplicate(a, &d_inv, p)
}
...
pub fn params_to_mod_check(a: &BigUint, b: &BigUint, p: &BigUint) -> Result<(), FiniteFieldError> {
    ...
    if !params_check {
        return Err(FiniteFieldError::InvalidArgument(format!(
            "a and b has to be greater than p: {}, {}, {}",
            a, b, p
        )));
    }
...
mod test {
    ...
    fn test_inverse_addition() {
        ...
        let res = inverse_addition(&a, &p).unwrap();
        ...
        assert_eq!(
            inverse_addition(&a, &p),
            Err(FiniteFieldError::InvalidArgument(format!(
                "a and b has to be greater than p: {}, {}",
                a, p
            )))
        );
        ...
        let c_inv = inverse_addition(&a, &p);

        assert_eq!(c_inv, Ok(BigUint::from(47u32)));
        assert_eq!(add(&a, &c_inv.unwrap(), &p), Ok(BigUint::from(0u32)));
    }
