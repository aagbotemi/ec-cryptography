...
impl EllipticCurve {
    ...
    pub fn add(&self, a: &CurvePoint, b: &CurvePoint) -> Result<CurvePoint, EllipticCurveError> {
        ...
        match (a, b) {
            ...
            (CurvePoint::Coordinate(x1, y1), CurvePoint::Coordinate(x2, y2)) => {
                ...
                let y1plusy2 = finite_field::add(y1, y2, &self.p).unwrap();
                ...
                // s = (y2 - y1) / (x2 - x1) mod p
                let numerator = finite_field::subtract(y2, y1, &self.p).unwrap();
                let denominator = finite_field::subtract(x2, x1, &self.p).unwrap();

                let s = finite_field::divide(&numerator, &denominator, &self.p).unwrap();
    ...
    pub fn double(&self, a: &CurvePoint) -> Result<CurvePoint, EllipticCurveError> {
        ...
        match a {
            ...
            CurvePoint::Coordinate(x1, y1) => {
                ...
                let numerator =
                    finite_field::multiplicate(&BigUint::from(3u32), &numerator, &self.p).unwrap();
                let numerator = finite_field::add(&self.a, &numerator, &self.p).unwrap();

                let denominator =
                    finite_field::multiplicate(&BigUint::from(2u32), y1, &self.p).unwrap();
                let s = finite_field::divide(&numerator, &denominator, &self.p).unwrap();
    ...
    fn compute_x3_y3(
        ...
        let x3 = finite_field::subtract(&s2, x1, &self.p).unwrap();
        let x3 = finite_field::subtract(&x3, x2, &self.p).unwrap();

        let y3 = finite_field::subtract(x1, &x3, &self.p).unwrap();
        let y3 = finite_field::multiplicate(s, &y3, &self.p).unwrap();
        let y3 = finite_field::subtract(&y3, y1, &self.p).unwrap();
    ...
    pub fn scalar_mul(
        ...
        a: &CurvePoint,
        d: &BigUint,
    ) -> Result<CurvePoint, EllipticCurveError> {
    ...
    pub fn is_on_curve(&self, a: &CurvePoint) -> bool {
        match a {
            CurvePoint::Coordinate(x, y) => {
                ...
                let ax = finite_field::multiplicate(&self.a, x, &self.p).unwrap();
                let x3plusax = finite_field::add(&x3, &ax, &self.p).unwrap();

                y2 == finite_field::add(&x3plusax, &self.b, &self.p).unwrap()
            }
...
mod test {
    ...
    fn test_point_addition() {
        ...
        let p1 = CurvePoint::Coordinate(BigUint::from(6u32), BigUint::from(3u32));
        let p2 = CurvePoint::Coordinate(BigUint::from(5u32), BigUint::from(1u32));
        let pr = Ok(CurvePoint::Coordinate(
            BigUint::from(10u32),
            BigUint::from(6u32),
        ));
